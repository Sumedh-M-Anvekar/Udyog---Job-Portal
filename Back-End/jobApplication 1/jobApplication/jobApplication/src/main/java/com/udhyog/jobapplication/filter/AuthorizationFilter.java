package com.udhyog.jobapplication.filter;
...
import com.udhyog.jobapplication.util.JwtUtils;
    ...
    //Read the Authorization Header from the Request
          ...
          else
          {
              try {
                  ...
                  System.out.println("Payload " + JwtUtils.validateToken(token));
                  Claims claims = JwtUtils.validateToken(token);
 
                  String payload = claims.getSubject();
 
                  System.out.println("Payload " + payload);
                  httpServletRequest.setAttribute("emailId", payload);
                  filterChain.doFilter(servletRequest, servletResponse);
              }
              catch(Exception exe)
              {
